...
Personal, local-only monthly finance tracker for one user (Naman). Not a
...
into the repo root and wants them turned into rows matching the `Entries`
sheet of their existing hand-maintained `Expense Tracker.xlsx`, with known
merchants (Swiggy, Instamart, HungerBox, ...) auto-categorized and everything
else editable before it's saved. Their real statement files and real
`Expense Tracker.xlsx` live at the repo root — they are sample/reference data
_and_ the user's actual working files, not throwaway fixtures. Don't delete
or rewrite them; the app only ever _reads_ `Expense Tracker.xlsx` (once, to
seed the database on first run) and never writes to it.
...
server/src/
  ...
  rules.js       merchant-narration -> {type, category, description} matching engine
...
server/data/finello.db   the actual database, gitignored (see .gitignore at repo root —
                         it's a generic template that happens to include node_modules/;
                         if it's ever missing data/ specifically, add it, don't assume)
```
...
- `accounts (id, name UNIQUE, identifier UNIQUE)` — `identifier` is how a
  ...
  user every time (e.g. `hdfc_savings`, `card:3657`, `sbi_card:XX02`).
...
- `categories (id, type, name, tag, UNIQUE(type, name))` — `type` is the
  ...
  user's own `Settings` sheet "Type" column), `name` is what the user's
  sheet calls "Category" (Food, Groceries, ...), `tag` is Need/Want, only
  meaningful for Expense. **The user's "subcategory" language in the
  original ask maps to this `category` field** — there is no separate
  subcategory level; don't add one without being asked.
- `descriptions (name)` — flat list of merchant/description strings ever
  used, for the description dropdown. Seeded from every distinct
  `Description` value in the user's existing `Entries` sheet.
- `rules (id, pattern, direction, type, category, description, ignore,
priority)` — the auto-mapping engine's source of truth. `pattern` is
  ...
  rule applies to. `ignore=1` means matching rows default to excluded (used
  for card-bill payment lines that would double-count money already tracked
  on the bank side — see "CC bill payment" gotcha below). Higher `priority`
  wins; ties broken by longer pattern. Seeded once from `DEFAULT_RULES` in
  `seed.js` (derived by reverse-engineering the user's own historical
  Entries — HungerBox→Food, Instamart/Blinkit→Groceries, Swiggy→Food, etc.)
  and grows over time from "remember this mapping" in the Review UI.
...
5. User edits rows inline (`PATCH /api/transactions/:id`). If they change
   type/category/description on a row that wasn't auto-mapped and "Remember
   my mappings" is checked, `rememberMapping()` in `index.js` both
   upserts a `rules` row keyed on a normalized merchant key (derived from
   the narration — strips `UPI-` prefixes, `PAYU*`/`RAZORPAY*` prefixes,
   trailing city names — see `merchantKey()` in `rules.js`) **and**
   immediately reapplies it to other still-unmapped rows in the same batch.
...
## Bulk ledger import & reset (`/api/ledger/*`, `LedgerActions.jsx`)

Separate from the monthly statement flow above — this is how the user
bootstraps (or rebuilds) their full transaction history in one shot, and how
they wipe test/bad data without touching accounts/categories/rules:
...
- `POST /api/ledger/import` (single file, `ledgerXlsx.js`) — parses an
  ...
  ledger state and the Ledger toolbar, and again in Settings.
- `POST /api/ledger/reset` — deletes **all** `transactions` and `batches`
  rows (typed "RESET" confirmation required in the UI), but leaves
  `accounts`, `categories`, and `rules` alone, so the auto-mapping setup
  survives a reset and re-import. This is the "start over" button, not a
  factory reset — don't extend it to also clear rules/categories unless
  asked; that's a different, more destructive action.
...
- **Category vs. subcategory**: there is only one level (`category`) beneath
  `type`. If the user asks for a genuine second level, that's a schema
  change (new column + UI), not something to fake with string concatenation.
...
- `ImportLedgerButton`/`ResetLedgerButton` (`LedgerActions.jsx`) are shared
  between the Ledger and Settings pages on purpose (same destructive/bulk
  action, two entry points). If it needs to change, change it once there —
  don't fork a second copy into a page.
...
`curl` against the real sample statement files in the repo root (e.g. `curl
-F "files=@Acct_Statement_XXXXXXXX9069_31122025.xls" ... http://127.0.0.1:5174/api/imports`),
